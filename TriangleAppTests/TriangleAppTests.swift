...
class TriangleAppTests: XCTestCase {
  ...
  func testDetectRandomTriangle() {
    XCTAssertEqual(try detectTriangle(4, 5, 6), "Segitiga Sembarang")
  }
  
  func testDetectRandomTriangle2() {
    XCTAssertEqual(try detectTriangle(4, 5, 6), "Segitiga Sembarang")
  }
  ...
  func testPythagorasTriangle() {
    XCTAssertEqual(try detectTriangle(3, 4, 5), "Segitiga Siku-Siku")
  }
  ...
  func detectTriangle(
    ...
    } else if (Double(sides[0] * sides[0]) + Double(sides[1] * sides[1])).squareRoot() == Double(sides[2]) {
